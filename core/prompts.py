...
- **「良い点」「気になる点」を各{reviews_each}件**。実在しない購入者の体験談を一人称の「」でねつ造するのは禁止（景表法・ステマ規制）。スペックと一般的傾向から想定される利点・注意点を、編集部の見立てとして「〜な人には便利でしょう」「〜の場合は物足りないかも」の体で具体的に（使う場面・数字感はOK、作り話の引用はNG）。
