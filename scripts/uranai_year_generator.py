...
# 金運軸のラッキー要素（MDの金運開運法ベース。断定/誇大・必ず/絶対は使わない）
LUCKY_COLORS = ["ゴールド", "山吹色", "若草色", "ラベンダー", "パールホワイト", "深緑",
                "ターコイズ", "小豆色", "藤色", "珊瑚色", "白", "藍色"]
LUCKY_DIRS = ["東", "南東", "南", "北西", "西", "東南"]
LUCKY_NUMS = ["3", "5", "7", "8", "11", "15", "24", "8"]
# 締めは「呼びかけ × 開運アクション」を組み合わせて大量のバリエーションを作る。
# {e}=絵文字 {color}=色 {dir}=方位 {num}=数字
RANK_ADDR = [
    "上位の星座さんは、", "ランクインした人は、", "今日トップ5に入ったあなたは、",
    "1位さんは、", "1〜3位さんは、", "圏外だった人も大丈夫、", "この投稿が目に留まった人は、",
    "今日のランキングを見たあなたは、", "ピンと来た人は、",
]
RANK_ACT = [   # 呼びかけに続く金運アクション/メッセージ
    "お財布の中を整えると、金運の流れがもっと整います💰",
    "お札の向きをそろえてみて。巡りが変わる合図です✨",
    "新しい財布を迎えるなら今日が好機。種銭を入れて始めると流れに乗れます💰",
    "玄関にお花や緑をひとつ。良いご縁とお金の入口になります🌿",
    "{dir}の窓を開けて新しい気を取り入れて。お金とご縁が巡ります🪙",
    "今日のラッキーカラー{color}を小物に取り入れると、金運が動きやすくなります✨",
    "ラッキーナンバーは{num}。レシートや時刻にこの数字を見たら金運の合図💫",
    "財布から不要なレシートを出すだけで、金運の余白が生まれます💰",
    "水まわりをサッと磨くと、金運の巡りが整う日✨",
    "小さな「ありがとう」を3回となえてみて。豊かさの種になります🌱",
    "思いがけない豊かさの波が近づいているかも。受け取る準備を💰",
    "抱えていた重さが、ふっと軽くなる日。深呼吸をひとつ✨",
    "臨時収入の気配を見逃さないで。今日はアンテナを立てておいて💫",
    "{dir}まわりを少し片づけると、金運アップの合図に🪙",
    "通帳や残高を一度やさしく眺めて。お金に感謝すると巡りが良くなります💰",
    "今日手にしたお金に「ありがとう」を。豊かさは感謝に集まります✨",
]
RANK_STANDALONE = [   # 単独で完結する締め（呼びかけ無し）
    "今日「{e}」を置けた星座さんから、流れが動き出します",
    "ランクインした人、抱えていた重さがふっと軽くなる日✨ そっと{e}を置いて",
    "上位の星座さんは、宇宙からのギフトを受け取る準備を{e}",
    "見た人みんなに、今日いいことが巡りますように{e}",
    "順位より、あなたが「いいな」と思えたら、それが今日の開運サイン{e}",
    "迷ったら、心が軽くなる方を選んで。それが金運の近道です{e}",
]
...
def style_B_rank(seed: random.Random, d: date) -> str:
    ...
    raw = (seed.choice(RANK_ADDR) + seed.choice(RANK_ACT)) if seed.random() < 0.75 \
        else seed.choice(RANK_STANDALONE)
    tail = raw.format(e=e, color=seed.choice(LUCKY_COLORS),
                      dir=seed.choice(LUCKY_DIRS), num=seed.choice(LUCKY_NUMS))
